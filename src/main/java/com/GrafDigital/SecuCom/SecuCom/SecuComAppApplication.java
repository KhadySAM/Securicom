	...
	CommandLineRunner start(AccountService accountService){
		return  args -> {
			...
			if (appUserRepository.findAll().size() == 0) {
				...
				accountService.addNewUser(new AppUser(null, "Kadidia", "1234", new ArrayList<>()));

				// Attribuer des roles à l'Admin
				accountService.addRoleToUser("Kadidia", "USER");
				accountService.addRoleToUser("Kadidia", "ADMIN");
			}
		};
	}
